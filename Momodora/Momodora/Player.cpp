...
void Player::Init()
{
	...
	InitPlayerVar(); // ?åÎ†à?¥Ïñ¥Í∞Ä Í∞ÄÏß?Í∞ÅÏ¢Ö Î≥Ä?òÎì§ Ï¥àÍ∏∞???òÎäî ?®Ïàò
}
...
{
	// Î∂ÄÎ™??¥Îûò?§Ïù∏ GameObject???†Ïñ∏??Î≥Ä?òÎì§
	mName = "Player";
	mX = WINSIZEX / 2.f;
	mY = 200.f;
	...
	mHitBox = RectMakeCenter((int)mX, (int)mY, (int)(mSizeX / 3.f), (int)mSizeY); // ?ÑÏãú(Í∞ÄÎ°úÎßå 1/3)
	mAttackBox = RectMakeCenter((int)mX, (int)mY, (int)mSizeX, (int)mSizeY); // ?ÑÏãú(?åÎ†à?¥Ïñ¥ ?¨Ïù¥Ï¶?
...
void Player::SetEndAttack()
{
	...
	mLeaf->SetIsActive(false); // ?†ÎãàÎ©îÏù¥?????åÍ≥† Ï∂úÎ†•?????òÍ≤å
}
